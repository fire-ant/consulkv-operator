...
A helm folder will then be created under consulkv-Operator repo root, which is ready to be used by helm.
"""
import argparse, subprocess, re, shutil, semver
from pathlib import Path

parser = argparse.ArgumentParser(description='Variables to build helm chart')
...
out=subprocess.run(["make", "manifests", 'IMG=' + args.image + ":" + args.version], capture_output=True).stdout.decode("utf-8")
# split the output
contents=out.split('---\n')

name=re.search('/(.+?):', args.image)
# initialize the directories
helmdir=name.group(1)+"-helm"
...
if Path(crddir).exists():
        shutil.rmtree(crddir)
Path(crddir).mkdir(parents=True, exist_ok=True)
...
for content in contents:
    ...
    # first "name" found in the manifest. For example: consulkv
      name=re.findall('(?<=name: )\S+', content)[0]
      filename=kind + "-" + name + ".yaml"
      with open(crddir+"/"+filename, 'w') as f:
        f.write(content)
    
    # skip namespace and CRD
    if kind == "Namespace" or kind == "CustomResourceDefinition":
        continue
    name=re.findall('(?<=name: )\S+', content)[0]
    filename=kind + "-" + name + ".yaml"

    # replace namespace in content
    content=re.sub("consulkv-operator-system", '{{ .Release.Namespace }}', content)
...
valfile="hack/values.yaml"
if Path(valfile).exists():
    subprocess.run(["cp", valfile, helmdir])
else:
    exit(1)
